...
## Этап 14 — Reporting, Global Verification & Structured Interface

**Статус: Planned**

**Reporting:** добавить понятный человеку отчёт Discovery/Capture о том, какое
состояние поддерживается и захвачено, исключено или не поддерживается.

**Global Verification:** сводно проверять соответствие выбранного целевого
окружения после обычного Bootstrap/Workflow и, где применимо, после Restore.
Опираясь на существующий локальный цикл модулей `Check → Apply → Verify`,
агрегировать наблюдаемые результаты в единый результат окружения, не заменяя и
не дублируя локальный Verify. Различать подтверждённое соответствие,
несоответствие, предупреждения и неподдерживаемое состояние, а также ошибки
наблюдения и проверки; успешное завершение команды само по себе не доказывает
соответствие окружения выбранному состоянию.

**Structured Interface:** создать стабильный машиночитаемый интерфейс состояния
и результатов Core для будущего GUI. Он должен предоставлять результаты
Discovery, планирования/Preview, проверки и операций без разбора человеком
читаемых сообщений CLI или логов. Core остаётся источником решений о валидации,
...
Application Environment Expansion
  ↓
Reporting, Global Verification & Structured Interface
  ↓
Macseed GUI
  ↓
Secure Migration v2
