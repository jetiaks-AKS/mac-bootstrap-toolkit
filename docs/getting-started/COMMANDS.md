...
Проверить текущее состояние поддерживаемых компонентов.

```bash
./bootstrap.sh --check
```

`--check` не является полностью side-effect-free: он создаёт логи,
выполняет preflight-проверки и может запросить administrator
authentication. Если Homebrew отсутствует, Toolkit предложит его
установить.
