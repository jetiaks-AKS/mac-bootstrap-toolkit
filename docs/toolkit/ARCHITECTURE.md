...
Bootstrap Engine отвечает за восстановление рабочего окружения на новом Mac на основе автоматически сгенерированной конфигурации Discovery.

Bootstrap не анализирует систему самостоятельно. Все решения принимаются на основании конфигурационных файлов из `config/generated/`.

Каждый модуль сначала проверяет текущее состояние системы, затем при необходимости выполняет восстановление и повторно проверяет результат (Verify After Apply).
...
На текущий момент Workspace Bootstrap поддерживает:

- восстановление структуры Workspace;
- автоматическое восстановление отсутствующих Git-репозиториев;
- проверку Git-репозитория;
- проверку Remote URL;
- проверку текущей Git-ветки.

Workspace Bootstrap использует исключительно конфигурацию Discovery (`config/generated/workspace/`) и не анализирует систему самостоятельно.
